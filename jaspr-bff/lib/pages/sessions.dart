...
///
/// The Refresh button is the **BFF** half, and it is the only part of this file
/// that differs from `../jaspr-ssr`. It calls `GET /api/sessions`, an endpoint
/// declared in `main.server.dart`. Note what it is *not* needed for: the
/// initial render. SSR already put that data in the HTML. A BFF earns its place
/// only when the browser has to ask for something after the page is loaded.
@client
...
class SessionsState extends State<Sessions>
  ...
  /// Runs in the browser only — the server never needs this, because
  /// [preloadState] reads the repository directly.
  Future<void> refresh() async {
    ...
    setState(() {
      sessions = [
        for (final s in json['sessions'] as List<dynamic>) Session.fromJson(s as Map<String, dynamic>),
      ];
      refreshing = false;
    });
  }
    ...
    return section([
      ...
      if (kIsWeb)
        button(
          ...
          [.text(refreshing ? 'Refreshing…' : 'Refresh from /api/sessions')],
        ),
      ul(classes: 'sessions', [
